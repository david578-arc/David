  ...
  const fetchDashboardStats = async () => {
    try {
      ...
      if (user.role === 'FIFA_ADMIN') {
        endpoints.push(
          axios.get('http://localhost:8080/api/teams/analytics', config),
          axios.get('http://localhost:8080/api/players', config),
          axios.get('http://localhost:8080/api/matches', config)
        );
      } else if (user.role === 'TEAM_MANAGER' || user.role === 'COACH') {
        endpoints.push(
          axios.get(`http://localhost:8080/api/players/team/${user.team}`, config),
          axios.get('http://localhost:8080/api/matches', config)
        );
      }
