...
const PlayerDashboard = ({ user, stats, token }) => {
  ...
  const fetchPlayerData = async () => {
    ...
    try {
      ...
      const playersRes = await axios.get('http://localhost:8080/api/players', config);
      ...
      if (playerData) {
        ...
        // Fetch player stats
        const statsRes = await axios.get(`http://localhost:8080/api/players/${playerData.id}/stats`, config);
        setPlayer(prev => ({ ...prev, ...statsRes.data }));
      }

      // Fetch matches
      const matchesRes = await axios.get('http://localhost:8080/api/matches', config);
  ...
  const updatePlayerProfile = async (updates) => {
    ...
    try {
      ...
      await axios.put(`http://localhost:8080/api/players/${player.id}`, updates, config);
      fetchPlayerData();
    } catch (error) {
