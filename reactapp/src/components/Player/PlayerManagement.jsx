...
const PlayerManagement = ({ token }) => {
  ...
  const fetchPlayers = async () => {
    try {
      ...
      const response = await axios.get('https://8080-acbffafcdacceebbbdbcfdbdb.premiumproject.examly.io/api/players', config);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingPlayer) {
        await axios.put(`https://8080-acbffafcdacceebbbdbcfdbdb.premiumproject.examly.io/api/players/${editingPlayer.id}`, playerData, config);
      } else {
        await axios.post('https://8080-acbffafcdacceebbbdbcfdbdb.premiumproject.examly.io/api/players', playerData, config);
      }
  ...
  const handleDelete = async (id) => {
    if (window.confirm('Are you sure you want to delete this player?')) {
      try {
        ...
        await axios.delete(`https://8080-acbffafcdacceebbbdbcfdbdb.premiumproject.examly.io/api/players/${id}`, config);
        await fetchPlayers();
        setError('');
      } catch (err) {
        ...
        {filteredPlayers.map(player => (
          <div key={player.id} className="player-card">
            ...
            <div className="player-info">
              ...
              <p className="team">{player.team?.teamName || 'No Team'}</p>
